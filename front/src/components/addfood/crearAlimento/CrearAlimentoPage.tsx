...
export default function CrearAlimento(props:{screenSize:string, setscreenSize:any, soyNutri?:string}) 
{
  ...
  useEffect(() => 
  {
    ...
    getTamanyoPantalla(props.setscreenSize)
  ...
  const crearAlimento = async (idRecibo:number) => 
  {
    ...
    let nom = "";
    if(props.soyNutri)
      nom = sessionStorage.getItem("nutriNom")
    else
      ...
      const response = await axios.post(
        ...
        if(response.data != null)
        {
          ...
          const timer = setTimeout(() => {
            setbtnfinishedPulsado(false);
            if(props.soyNutri) 
                location.href = "../addfood/buscarAlimento?soyNutri=true"
            else
                location.href = "../addfood/buscarAlimento"
          }, 3000);
...
const goback = () =>
{
  ...
  return (
    ...
    {!props.soyNutri  &&<BarraMenu></BarraMenu>}
    {props.soyNutri &&<BarraMenuNutri></BarraMenuNutri>}
      ...
      <CustomCard mt={mensajeError==true ? "20px" : "0px"} hijo={ 
        <TitleCard title={'CREATE A FOOD'} soyNutri={props.soyNutri}
      ...
      {props.screenSize != "" && <CustomCard mt="10px" hijo={ 
      ...
      {props.screenSize != "" && <CustomCard mt="10px" hijo={ 
        <MacroNutrCardEdit recibo={recibo} setrecibo={setrecibo} 
        totalMacro={reciboConstNames.grasas} screenSize={props.screenSize} 
        infoLista={[reciboConstNames.monoinsaturadas,reciboConstNames.poliinsaturadas, reciboConstNames.saturadas]}>
        </MacroNutrCardEdit>}></CustomCard>}

      {props.screenSize != "" &&  <CustomCard mt="10px" hijo={  
      <MacroNutrCardEdit recibo={recibo} setrecibo={setrecibo} 
      totalMacro={reciboConstNames.carbs} screenSize={props.screenSize} infoLista={[reciboConstNames.complejos, reciboConstNames.simples]}>
      </MacroNutrCardEdit>}></CustomCard>}

      {props.screenSize != "" && <CustomCard mt="10px" hijo={ 
      <FiberCard edit={true} recibo={recibo}  setrecibo={setrecibo} totalFiber={recibo.fibra} screenSize={props.screenSize}></FiberCard>}></CustomCard>}


      </Flex>}

      {props.screenSize == "" && <PurpleSpinner></PurpleSpinner>}
    </>);
